...
describe('connectionService', () => {
  ...
  describe('getConnection', () => {
    ...
    it('should throw error for non-existent connection', async () => {
      await expect(connectionService.getConnection('nonexistent')).rejects.toThrow()
    })
  })
  ...
  describe('createConnection', () => {
    it('should create new connection', async () => {
      ...
      expect(requests.createConnection).toEqual({ server_name: 'US-West', months: 1 })
    })
  })

  describe('updateConnection', () => {
    it('should update connection settings', async () => {
      const connection = await connectionService.updateConnection('testuser-1', {
        auto_renew: false,
      })
  ...
  describe('toggleAutoRenew', () => {
    it('should toggle auto-renew for connection', async () => {
      const connection = await connectionService.toggleAutoRenew('testuser-1', false)
  ...
  describe('toggleEnabled', () => {
    it('should toggle enabled state for connection', async () => {
      const connection = await connectionService.toggleEnabled('testuser-1', false)
  ...
  describe('changeServer', () => {
    it('should change server for connection', async () => {
      const connection = await connectionService.changeServer('testuser-1', 'EU-Central')
