...
import { staggerItem, pageVariants } from '../styles/animations'
    ...
    >
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 0.5 }}>
          ...
          >
            {isFromSupport ? <SupportAgentIcon sx={{ fontSize: 14 }} /> : <ChatIcon sx={{ fontSize: 14 }} />}
          </Avatar>
...
const NewConversationDialog = ({
  ...
  const handleSubmit = async () => {
    ...
    setError('')
    try {
      const conversation = await supportService.createConversation(message.trim())
...
const Support = () => {
  ...
  const handleCloseConversation = async () => {
    ...
    try {
      const updated = await supportService.closeConversation(selectedConversation.id)
      setSelectedConversation(updated)
      fetchConversations()
    } catch (err) {
  ...
  const renderMessage = (message: SupportMessage, isUser: boolean) => {
    if (chatVariant === 'telegram') {
      return <TelegramBubble key={message.id} message={message} isUser={isUser} />
    }
              ...
              {selectedConversation ? (
                <>
                  ...
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      {isMobile && (
                        <IconButton onClick={() => setSelectedConversation(null)} size="small">
                          <ArrowBackIcon />
                        </IconButton>
                      )}
                  ...
                  <Box
                    sx={{
                      ...
                      overflow: 'auto',
                      p: 2,
                      bgcolor: chatVariant === 'telegram' 
                        ? (theme.palette.mode === 'dark' ? 'grey.900' : '#e8f0fe')
                        : 'background.default',
                    }}
                  >
