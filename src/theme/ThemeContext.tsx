...
import type { ThemeName, TelegramThemeParams } from './types'
...
function getInitialTheme(): ThemeName {
  ...
  if (stored && ['violet', 'teal', 'telegram', 'light', 'dark'].includes(stored)) {
    return stored as ThemeName
  }
  return 'violet'
}

function detectTelegramTheme(): { isTelegram: boolean; params: TelegramThemeParams | null; isDark: boolean } {
...
export function CustomThemeProvider({ children }: CustomThemeProviderProps) {
  ...
  const [telegramState, setTelegramState] = useState<{ isTelegram: boolean; params: TelegramThemeParams | null; isDark: boolean }>({
    isTelegram: false,
    params: null,
    isDark: false,
  })
    ...
    switch (themeName) {
      ...
      case 'telegram':
        if (telegramState.isTelegram && telegramState.params) {
          return createTelegramTheme(telegramState.params, telegramState.isDark)
        }
  ...
  const contextValue: ThemeContextValue = {
    themeName,
    setThemeName,
    isDark: themeName === 'telegram' ? telegramState.isDark : themeName === 'dark',
